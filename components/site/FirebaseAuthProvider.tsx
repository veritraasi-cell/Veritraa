    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-[#120b07]/70 px-4 py-6 backdrop-blur-sm">
      <div className="relative w-full max-w-lg overflow-hidden rounded-[2rem] border border-[#e5c7af] bg-[linear-gradient(180deg,#fff9f2,#fff2e4)] shadow-[0_24px_70px_-28px_rgba(82,28,4,0.6)]">
        ...
        <div className="px-6 pb-6 pt-8 sm:px-8 sm:pb-8 sm:pt-10">
          <p className="text-[11px] font-bold uppercase tracking-[0.28em] text-[#99461e]">Customer access</p>
          <h2 className="mt-3 font-headline text-3xl text-[#1d1c18]">
            {mode === 'profile' ? 'Finish your profile' : 'Login with Google'}
          </h2>
          <p className="mt-3 text-sm leading-6 text-[#55433b]">
            {mode === 'profile'
              ? 'We need a mobile number so your comments, likes, and order history are linked to your customer record.'
              : 'Sign in once to browse the shop, add items to cart, leave comments, and store your order details in Firebase.'}
          </p>

          {!hasFirebaseClientConfig() ? (
            <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
              Firebase client config is missing. Add the NEXT_PUBLIC_FIREBASE_* values in .env.local.
            </div>
          ) : null}
          ...
          <div className="mt-6 space-y-4">
            ...
            <button
              ...
              disabled={authBusy || !hasFirebaseClientConfig() || (mode === 'profile' && phone.trim().length < 6)}
              onClick={() => {
                if (mode === 'profile') {
              ...
              type="button"
            >
              {authBusy ? 'Please wait...' : mode === 'profile' ? 'Save mobile number' : 'Login with Google'}
            </button>

            {mode === 'login' ? (
              <p className="text-center text-xs leading-5 text-[#7e5a4b]">
                A Google popup will open when you tap the button. Browser policies do not allow opening it without a click.
              </p>
            ) : null}
          </div>
        </div>
...
export function FirebaseAuthProvider({ children }: Readonly<{ children: ReactNode }>) {
    ...
    async function bootstrap() {
      try {
        ...
        if (session) {
          ...
          setDialogOpen(!session.phone?.trim());
        } else {
          setCustomer(null);
        ...
        if (!cancelled) {
          setAuthError(error instanceof Error ? error.message : 'Unable to load customer session.');
          setStatus('unauthenticated');
          setDialogMode('login');
          setDialogOpen(true);
        }
  ...
  async function syncCustomerSession(idToken: string, phone?: string | null) {
    ...
    if (!response.ok || !payload.data?.customer) {
      throw new Error(payload.error?.message ?? 'Unable to sign in with Firebase.');
    }

    setCustomer(payload.data.customer);
    setStatus('authenticated');
    setDialogMode(payload.data.customer.phone?.trim() ? 'login' : 'profile');
    setDialogOpen(!payload.data.customer.phone?.trim());
    router.refresh();
  ...
  async function signInWithGoogle() {
    ...
    try {
      ...
      setAuthError(error instanceof Error ? error.message : 'Unable to continue with Google sign-in.');
      setDialogOpen(true);
    } finally {
      setAuthBusy(false);
    }
